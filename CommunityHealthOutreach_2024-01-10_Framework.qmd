...
This table is the record of all living individuals who have an active enrollment with BCBS HCA ACC health plan who are auto-assigned to an Alliance Provider, and who have a residential address in the northern GSA counties. In other words, this is a roster of all active Alliance ACO members.
...
AllianceEnrolledMembers_Merge <-
  merge(x = AllianceEnrolledMembers_Merge,
        y = GM_ALLAZ_Enrollments, # Adds AHCCCSID
        by = "PrimaryId",
        all.x = TRUE)
...
AllianceEnrolledMembers_Merge <- 
  AllianceEnrolledMembers_Merge |> 
  filter(Void == FALSE) |> # filters out any voided entries
  select(
    PrimaryId,
    AHCCCSID,
    LastName,
    ...
    ResAddr1, # Senior finance leaders report this field may not be 100% reliable
    ResCity, # Senior finance leaders report this field may not be 100% reliable
    ResEffDate,
    HomePhone,
    EmergencyPhone,
    EmailAddress
...
objects_to_keep <- c("Data_VBPQR_AllRecords",
                     ...
                     "NAZ_Counties",
                     "Provider_ShirtName",
                     "ContractMeasures")  # Add more objects to keep as needed
