...
Filter the AllHCA claims table for only those people on the Alliance Enrolled Members roster. Then attach the CHO Roster data to each claim.
...
collapsed_data <-
  collapsed_data |> 
  mutate(NoSvcs = if_else(SvcByAssignedBHHTIN + SvcByOtherTIN == 0, 1, 0))
...
Checkit <- CHO_Roster1 |> select(1, 40:61)
```
...
Checkit <- CHO_Roster1 |> select(1, 40:62)
```
...
Checkit <- CHO_Roster1 |> select(1, 40:63)
```
...
Checkit <- CHO_Roster1 |> select(1, 21:24, 40:67)
```
